...
parser.add_argument("-a", "--algo", help="Algorithm to include", type=str, required=True, nargs="+")
...
args.algo = [algo.upper() for algo in args.algo]
algos = args.algo
...
for algo in algos:
    ...
    for folder in dirs:
        ...
        if x.shape[0] >= args.episode_window:
            ...
            plt.plot(x, y_mean, linewidth=2, label=algo)
